...
CCalendarCtrl::CCalendarCtrl()
{
	...
	m_SelectionRange[0]	= COleDateTime(time_t(0));
	m_SelectionRange[1]	= COleDateTime(time_t(0));
	m_SelectionRange[2]	= COleDateTime(time_t(0));
	m_nVscrollMax		= 110;
	m_nVscrollPos		= m_nVscrollMax/2;
	m_BoundDown			= COleDateTime(1000,12,31,0,0,0); 
	m_BoundUp			= COleDateTime(3000,1,1,0,0,0);
	m_nMaxSel			= 42;
	m_nFirstWeekDay		= 1;
	m_pfnDataCallback	= NULL;

	menu.CreatePopupMenu();
	menu.AppendMenu(MF_STRING,IDM_SHOWEVENTS,_T("显示今天日程"));
	menu.AppendMenu(MF_STRING,IDM_EDITEVENTS,_T("修改今天日程"));
	menu.AppendMenu(MF_SEPARATOR);
	menu.AppendMenu(MF_STRING,IDM_MARKDAY,_T("标记今天"));
	menu.AppendMenu(MF_STRING,IDM_CLEAREVENTS,_T("清除今天日程！"));
	menu.AppendMenu(MF_SEPARATOR);
	menu.AppendMenu(MF_STRING,IDM_SETWEEKDAY,_T("设置起周日"));
}
...
void CCalendarCtrl::Reset()
{
	...
	// Reset selection
	m_SelectionRange[1] = COleDateTime(today.GetYear(), today.GetMonth(), today.GetDay(),0,0,0);
		...
		(dtDate > m_BoundDown) )
	{
		...
		for(int i=0; i<CALENDAR_ROWS ; i++)
		{
			for(int u=0; u<CALENDAR_COLUMNS; u++)
			{
				...
				{
					m_pfnDataCallback(this,cmdMark, m_dayCells[i][u].date);
					m_pfnDataCallback(this,cmdLoad, m_dayCells[i][u].date);
				}
				dtIte += COleDateTimeSpan(1,0,0,0);
			}
		}
		...
		if(bSelect)
		{
			// Callback to tell Parent Window Current Date focused or reset
			// for this Kind of Call back,Parameter 1 HWnd set as NULL
			m_pfnDataCallback(this,cmdTimeChanged, dtDate);

			m_RandomSelection.RemoveAll();
			m_SelectionRange[2] = COleDateTime(time_t(0));
			m_SelectionRange[1] = COleDateTime(m_DateCurrent.GetYear(), m_DateCurrent.GetMonth(), m_DateCurrent.GetDay(),0,0,0);
			m_SelectionRange[0] = m_SelectionRange[1];
	...
	for(int i=0; i<CALENDAR_ROWS ; i++)
	{
		for(int u=0; u<CALENDAR_COLUMNS ; u++)
		{
			...
			if(GetRectFromCell(i, u, rect))
			{
				...
				COleDateTime tmax = max(m_SelectionRange[0], m_SelectionRange[1]);
				COleDateTime tmin = min(m_SelectionRange[0], m_SelectionRange[1]);			
				COleDateTime tcur = m_dayCells[i][u].date;				
				if(m_RandomSelection.GetCount())
				{	
					POSITION pos = m_RandomSelection.GetStartPosition();	 
					while (pos){
						void* p; COleDateTime date;
						m_RandomSelection.GetNextAssoc(pos, date, p);
						if(date == tcur)
						{							
							CBrush br;
...
void CCalendarCtrl::GetLastSelectedGridCell(int &nRow, int &nCol)
{
	...
	for(int i=0; i<CALENDAR_ROWS ; i++)
	{
		for(int u=0; u<CALENDAR_COLUMNS ; u++)
		{
			if(m_SelectionRange[1] == m_dayCells[i][u].date){
				nRow = i; nCol = u;
				return;
			}
		}
...
void CCalendarCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(GetGridCellFromPoint(point, nRow, nCol) && !bCtrl)
	{
			...
			(m_dayCells[nRow][nCol].date > m_BoundDown) )
		{
			// Callback to tell Parent Window Current Date focused or reset
			// for this Kind of Call back,Parameter 1 HWnd set as NULL
			m_pfnDataCallback(this,cmdTimeChanged,m_dayCells[nRow][nCol].date );
			
			m_RandomSelection.RemoveAll();
			m_SelectionRange[2] = COleDateTime(time_t(0));
			m_bSelectionStarted = true;
			m_SelectionRange[0] = m_dayCells[nRow][nCol].date;
...
void CCalendarCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if(m_bSelectionStarted && !bCtrl)
	{
		...
		if(GetGridCellFromPoint(point, nRow, nCol))
		{
			if( (m_dayCells[nRow][nCol].date < m_BoundUp) && 
				(m_dayCells[nRow][nCol].date > m_BoundDown) )			
				m_SelectionRange[1] = m_dayCells[nRow][nCol].date;
			else
				m_SelectionRange[1]=m_SelectionRange[0];
		}
		else
		...
		if(GetGridCellFromPoint(point, nRow, nCol))
		{
				...
				(m_dayCells[nRow][nCol].date > m_BoundDown) )	
			{
				...
				{				
						...
						void*p;
						if(m_RandomSelection.Lookup(m_SelectionRange[1], p))
							m_RandomSelection.RemoveKey(m_SelectionRange[1]);
						else{
							if(m_RandomSelection.GetCount() < m_nMaxSel)
								m_RandomSelection.SetAt(m_SelectionRange[1], 0);
						}
		...
		case VK_SPACE:
			{
				if(bCtrl)
				{
					...
					if( (m_dayCells[nRow][nCol].date < m_BoundUp) && 
						(m_dayCells[nRow][nCol].date > m_BoundDown) )	
					{
						void*p; COleDateTime t = COleDateTime(m_dayCells[nRow][nCol].date);
						if(m_RandomSelection.Lookup(t ,p))
							m_RandomSelection.RemoveKey(t);
						else{
							if(m_RandomSelection.GetCount() < m_nMaxSel)
								m_RandomSelection.SetAt(t, 0);
						}
						Invalidate(true);
						return TRUE;
					}
					...
					(m_dayCells[nRow][nCol].date > m_BoundDown) )	
				{
					if(!bCtrl){
						...
						if(m_RandomSelection.GetCount() == 0)
							m_RandomSelection.SetAt(m_SelectionRange[0], 0);
					}
					...
					if( (m_dayCells[nRow][nCol].date < m_BoundUp) && 
						(m_dayCells[nRow][nCol].date > m_BoundDown) )	
					{
						m_SelectionRange[1] = COleDateTime(m_dayCells[nRow][nCol].date);
						if(bCtrl)
							m_SelectionRange[2] = m_SelectionRange[1];
...
int CCalendarCtrl::GetSelectedItems(CArray<COleDateTime,COleDateTime&> & dwaSelection)
{
	if(m_RandomSelection.GetCount())
	{
		POSITION pos = m_RandomSelection.GetStartPosition();
...
CCalendarCell* CCalendarCtrl::GetCell(COleDateTime date)
{
	for(int i=0; i<CALENDAR_ROWS ; i++)
		for(int u=0; u<CALENDAR_COLUMNS; u++)
			if(m_dayCells[i][u].date == date)
...
void CCalendarCtrl::AdjustSelection()
{
	// Callback to tell Parent Window Current Date focused or reset
	// for this Kind of Call back,Parameter 1 HWnd set as NULL
	m_pfnDataCallback(this,cmdTimeChanged,m_SelectionRange[1] );
...
void CCalendarCtrl::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, point.x, point.y,this->GetParent());

}
BOOL CCalendarCtrl::invokeCallback(int cmd)
{
	switch(cmd)
	{
	case cmdTimeChanged:
		m_pfnDataCallback(this,cmdTimeChanged,m_DateCurrent);
		break;
	case cmdLoad:
		m_pfnDataCallback(NULL,cmdLoad,m_DateCurrent);
		UpdateCells();
		break;
	case cmdSave:
		m_pfnDataCallback(this,cmdSave,m_DateCurrent);
		UpdateCells();
		break;
	case cmdClear:
		m_pfnDataCallback(this,cmdClear,m_DateCurrent);
		UpdateCells();
		break;
	case cmdTruncate:
		m_pfnDataCallback(this,cmdTruncate,m_DateCurrent);
		UpdateCells();
		break;
	}
	return TRUE;
